 ...
 **************************************************************/
// --(A) FULL PAGE Background switching via pageBgRadios--
pageBgRadios.forEach((radio) => {
  radio.addEventListener('change', () => {
    ...
    // Optional: fix the background so it doesn't scroll
    // document.body.style.backgroundAttachment = 'fixed';
  });
});

// --(B) Images for the CANVAS background--
const bg1 = new Image();
bg1.src = 'assets/canvasImages/bgCanvas1.jpg'; // <-- Make sure these paths are correct
const bg2 = new Image();
...
let characterPos = parseInt(positionSlider.value, 10); // starting position

/**************************************************************
 * 4. Event listeners
 **************************************************************/
// --(A) Radio buttons for the CANVAS background--
...
// --(B) Slider for character movement--
positionSlider.addEventListener('input', () => {
  characterPos = parseInt(positionSlider.value, 10);
  drawScene();
});

// --(C) Checkboxes for item toggling--
...
function drawScene() {
  ...
  // Draw the character
  // For example, x = characterPos, y = 300, width=50, height=50
...
// Wait for page (and images) to load, then draw the initial scene
